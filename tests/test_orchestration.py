...
-----------
  ...
  TestRouterNode       — router node with mocked OpenAI
  TestGeneratorNode    — generator node with mocked OpenAI
  TestRewriterNode     — rewriter node with mocked OpenAI
...
def _base_state(**overrides) -> dict:
    """Return a minimal valid RAGState dict."""
    state = {
        "query": "What is RAG?",
...
class TestRAGState:
    ...
    def test_original_query_preserved_after_update(self):
        ...
        # Simulate the rewriter changing query but NOT original_query
...
class TestRoutingFunctions:
    def test_route_after_router_returns_retriever_for_retrieve(self):
        ...
        state = _base_state(router_decision="retrieve")
        assert route_after_router(state) == "retriever"

    def test_route_after_router_returns_generator_for_conversational(self):
        from orchestration.graph import route_after_router
        state = _base_state(router_decision="conversational")
        assert route_after_router(state) == "generator"
    ...
    def test_route_after_generator_returns_end_when_confidence_high(self):
        ...
        state = _base_state(confidence_score=0.8, retry_count=0)
        assert route_after_generator(state) == END

    def test_route_after_generator_returns_end_at_exactly_04(self):
        from orchestration.graph import route_after_generator
        state = _base_state(confidence_score=0.4, retry_count=0)
        assert route_after_generator(state) == END

    def test_route_after_generator_returns_end_when_max_retries_reached(self):
        """Retry cap: even with low confidence, stop at retry_count == 2."""
        from orchestration.graph import route_after_generator
        state = _base_state(confidence_score=0.1, retry_count=2)
        assert route_after_generator(state) == END

    def test_route_after_generator_returns_rewriter_when_low_confidence(self):
        from orchestration.graph import route_after_generator
        state = _base_state(confidence_score=0.2, retry_count=0)
        assert route_after_generator(state) == "rewriter"

    def test_route_after_generator_returns_rewriter_at_retry_count_1(self):
        from orchestration.graph import route_after_generator
        state = _base_state(confidence_score=0.1, retry_count=1)
        assert route_after_generator(state) == "rewriter"
...
class TestRouterNode:
    def _call_router(self, json_content: str, query: str = "What is RAG?") -> dict:
        with patch("orchestration.nodes.router.openai") as mock_oai:
            mock_oai.OpenAI.return_value.chat.completions.create.return_value = (
                _mock_openai_response(json_content)
            )
            from orchestration.nodes.router import router_node
            return router_node(_base_state(query=query))

    def test_sets_retrieve_decision(self):
        result = self._call_router('{"decision": "retrieve"}')
        assert result["router_decision"] == "retrieve"

    def test_sets_conversational_decision(self):
        result = self._call_router('{"decision": "conversational"}')
        assert result["router_decision"] == "conversational"

    def test_defaults_to_retrieve_on_unexpected_value(self):
        result = self._call_router('{"decision": "unknown_value"}')
        assert result["router_decision"] == "retrieve"

    def test_defaults_to_retrieve_on_malformed_json(self):
        result = self._call_router("not valid json at all")
        assert result["router_decision"] == "retrieve"

    def test_returns_only_router_decision_key(self):
        result = self._call_router('{"decision": "retrieve"}')
...
class TestGeneratorNode:
    def _call_generator(
        ...
        with patch("orchestration.nodes.generator.openai") as mock_oai:
            mock_oai.OpenAI.return_value.chat.completions.create.return_value = (
    ...
    def test_parses_answer_field(self):
        result = self._call_generator(answer="RAG combines retrieval and generation.")
        assert result["answer"] == "RAG combines retrieval and generation."

    def test_parses_source_chunk_ids(self):
        result = self._call_generator(source_ids=["abc", "def"])
        assert result["source_chunk_ids"] == ["abc", "def"]

    def test_parses_confidence_score(self):
        result = self._call_generator(confidence=0.72)
        assert abs(result["confidence_score"] - 0.72) < 1e-6

    def test_updates_all_three_state_fields(self):
    ...
    def test_confidence_stored_as_float(self):
        result = self._call_generator(confidence=0.9)
...
class TestRewriterNode:
    def _call_rewriter(
        ...
        with patch("orchestration.nodes.rewriter.openai") as mock_oai:
            ...
            return rewriter_node(state)

    def test_increments_retry_count_by_one(self):
        result = self._call_rewriter(retry_count=0)
        assert result["retry_count"] == 1

    def test_increments_from_nonzero_retry_count(self):
        result = self._call_rewriter(retry_count=1)
        assert result["retry_count"] == 2

    def test_sets_rewritten_query_to_nonempty_string(self):
        result = self._call_rewriter(rewritten="Better phrasing of the query")
        assert isinstance(result["rewritten_query"], str)
        assert len(result["rewritten_query"]) > 0

    def test_rewritten_query_matches_llm_output(self):
        result = self._call_rewriter(rewritten="Specific alternative query")
...
class TestIntegration:
    ...
    def _run_graph(
        ...
        """Build and invoke the graph with controlled mock responses."""
        # Router
        router_payload = json.dumps({"decision": router_decision})

        # Generator — low confidence on first call if testing retry loop
        ...
        gen_responses = (
            [_mock_openai_response(gen_low), _mock_openai_response(gen_low), _mock_openai_response(gen_high)]
            if max_retries_hit
            else [_mock_openai_response(gen_high)]
        )
        ...
        with (
            ...
            patch("orchestration.nodes.generator.openai") as mock_gen_oai,
            ...
            patch("orchestration.app._app", None),  # force fresh compile
        ):
            mock_router_oai.OpenAI.return_value.chat.completions.create.return_value = (
                _mock_openai_response(router_payload)
            )
            mock_gen_oai.OpenAI.return_value.chat.completions.create.side_effect = gen_responses
            mock_rew_oai.OpenAI.return_value.chat.completions.create.return_value = (
                _mock_openai_response(rewriter_payload)
            )
            MockRetriever.return_value.retrieve.return_value = [
                _mock_retrieval_result()
            ]

            # Reset module-level retriever cache
            import orchestration.nodes.retriever_node as rn_mod
            rn_mod._retriever = None

            from orchestration.app import run_query
            return run_query("What is RAG?")
    ...
    def test_retrieve_path_populates_answer(self):
        result = self._run_graph(router_decision="retrieve", confidence=0.9)
        assert result["answer"] == "Final answer."
        assert result["router_decision"] == "retrieve"

    def test_conversational_path_bypasses_retriever(self):
        with (
            patch("orchestration.nodes.router.openai") as mock_router_oai,
            patch("orchestration.nodes.generator.openai") as mock_gen_oai,
            patch("orchestration.nodes.retriever_node.Retriever") as MockRetriever,
            ...
            mock_router_oai.OpenAI.return_value.chat.completions.create.return_value = (
                _mock_openai_response('{"decision": "conversational"}')
            )
            mock_gen_oai.OpenAI.return_value.chat.completions.create.return_value = (
                _mock_openai_response(
                    '{"answer": "Hello!", "source_chunk_ids": [], "confidence": 0.95}'
                )
            )
            ...
            # Retriever must not have been instantiated or called
            MockRetriever.assert_not_called()

    def test_fallback_loop_capped_at_two_retries(self):
        result = self._run_graph(max_retries_hit=True)
        assert result["retry_count"] <= 2

    def test_confidence_score_in_result(self):
        result = self._run_graph(confidence=0.88)
        assert isinstance(result["confidence_score"], float)
